...
public class MyCameraControl : MonoBehaviour
{
    ...
    public enum CameraTargets { PLAYER, TARGET, ARENA_CENTER, BATTLE_CENTER, TARGETandPLAYER_CENTER }
    public CameraTargets targetType;
    ...
    void Start()
    {
        ...
        //transformTargetPos = GameEngine.current.playerCharacter.transform.position;
        //transformTargetPos = GameEngine.globalArenaBounds.transform.position;
    }
    ...
    void Translate()
    {
        if (!ignoreTranslate)
        {
            ...
            //if (transformTargetPos == null) { return; }
            transform.position += Vector3.Scale((targetPosition + (Quaternion.LookRotation(new Vector3(transform.forward.x, 0f, transform.forward.z), Vector3.up) * transformOffset) - transform.position), translateSpeed);
        }
    }
    ...
    void OrbitView()
    {
        ...
        if (!ignoreRotate)
        {
            switch (targetType)
            {
                ...
                case CameraTargets.TARGET: transform.rotation = Quaternion.Euler(orbV, orbH, 0); break;
                    //case CameraTargets.TARGET: transform.rotation = Quaternion.LookRotation(targetPosition - transform.position, Vector3.up) * Quaternion.Euler(orbV, 0, 0); break;
            }


            if (focusTarget) { transform.rotation = Quaternion.LookRotation(targetPosition - transform.position, Vector3.up) * Quaternion.Euler(orbV, 0, 0); }
        }
    }
    ...
    public float LookAtOffset()
    {
        ...
        Vector2 newLook = new Vector2(targetPosition.x, targetPosition.z) - new Vector2(rig.transform.position.x, rig.transform.position.z);
    ...
    {
        //if(transformTargetPos == null) { return camDistance; }
        for (int i = 0; i < cameraClipPoints.Length; i++)
        {
            Ray ray = new Ray(targetPosition, cameraClipPoints[i] - targetPosition);
            float cDistance = Vector3.Distance(cameraClipPoints[i], targetPosition);
            if (Physics.Raycast(ray, cDistance, collisionLayer))
            { return GetAdjustedDistance(targetPosition); }
        }
        return camDistance;
    }
    ...
    void FixedUpdate()//LateUpdate()
    {
        ...
        if(targetType == CameraTargets.BATTLE_CENTER) 
        { 
            //collisionDistance = CitadelDeep.battleArenaSize * zoomMod;
            collisionDistance = Mathf.Clamp(collisionDistance, zoomMin, zoomMax);
        }
    ...
    public void ToggleTargeting()
    {
        switch(targetType)
        { 
            case CameraTargets.PLAYER: targetType = CameraTargets.TARGET; break;
            case CameraTargets.TARGET: targetType = CameraTargets.PLAYER; break;
        }
    }
}
